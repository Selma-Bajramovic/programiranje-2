...
public:
	...
	void AddElement(T1 el1, T2 el2, int lokacija) {
		if (*_trenutno == max)
			throw exception("Dostegli ste max");
...
public:
	...
	bool AddOcjena(int ocjena, Datum datum) {
		...
		{
			int razlika = datum.GetBrojDana() - _ocjene.getElement2(i)->GetBrojDana();
	...
	friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
		...
		cout << "Prosjek: " << obj.GetProsjekTehnike() << endl;
		return COUT;
	}
};
class Polaganje {
...
public:
	...
	float GetProsjekPojasa()const {
		...
		for (size_t i = 0; i < _polozeneTehnike.size(); i++)
		{
			suma += _polozeneTehnike[i]->GetProsjekTehnike();
		}
...
public:
	...
	*/
	bool AddTehniku(Pojas pojas, Tehnika tehnika) {
		if (pojas > ZUTI) {
			bool postojiNiziPojas = false;

			for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
			{
				if (_polozeniPojasevi[i]->GetPojas() == pojas - 1) {
					postojiNiziPojas = true;
					...
					{
						if (_polozeniPojasevi[i]->GetTehnike()[j]->GetProsjekTehnike() < 3.5)
							return false;
					}
				}
			}
		...
		for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
		{
			if (_polozeniPojasevi[i]->GetPojas() == pojas) {
				...
				thread thr(&KaratePolaznik::PosaljiEmail, this, tehnika, pojas, _polozeniPojasevi[i]->GetProsjekPojasa());
				thr.join();
		...
		thread thr(&KaratePolaznik::PosaljiEmail, this, tehnika, pojas, polaganje->GetProsjekPojasa());
		thr.join();

		return true;
	}

	void PosaljiEmail(Tehnika tehnika, Pojas pojas, float prosjekPojasa) {
		m.lock();
		cout << "FROM:info@karate.ba" << endl;
		cout << "TO: " << GetEmail() << endl;
		cout << "Postovani " << GetImePrezime() << ", evidentirana vam je tehnika " << tehnika.GetNaziv() << " za " << pojas << " pojas." << endl;
		cout << "Dosadasnji uspjeh (prosjek ocjena) na pojasu " << pojas << " iznosi " << prosjekPojasa << ", a ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi " << GetUkupniProsjek() << endl;
		cout << "Pozdrav." << endl;
	...
	float GetUkupniProsjek()const {
		...
		for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
		{
			suma += _polozeniPojasevi[i]->GetProsjekPojasa();
		}
...
const char* GetOdgovorNaPrvoPitanje() {
	...
	return "Odgovor -> const_cast koristimo kako bi pokazivaè ili referencu koja pokazuje na konstantnu vrijednost pretvorili u onu koja pokazuje na varijabilnu vrijednost";
}
const char* GetOdgovorNaDrugoPitanje() {
	...
	return "Odgovor -> Prvo se kreira objekat bazne klase za svaki objekat koji direktno nasljeðuje baznu klasu, potom se kreira naizvedeniji objekat koji nasljeðuje direktne nasljednike bazne klase";
}
void main() {
